...
suptitle = f'Effect of {primary_effect_to_vary} -- Various {secondary_effect_to_vary} ('
for param, val in xvar_to_fix.items():
    suptitle += f'{param}: {val},'
suptitle = suptitle[:-1] + ')'
...
for col_idx, yvar in enumerate(var_to_plot): 
    ...
    for row_idx, secondary_effect in enumerate(secondary_effect_group_list):
        ...
        df_filt_byseceffect.boxplot(column=yvar_col, by=primary_effect_to_vary, ax=ax[row_idx, col_idx], fontsize=14, grid=False, rot=0)
        ax[row_idx, col_idx].set_ylabel(yvar, fontsize=18)
        ax[row_idx, col_idx].set_xlabel('', fontsize=16)
        ax[row_idx, col_idx].set_title(secondary_effect, fontsize=16)
        ax[row_idx, col_idx].set_title(None)
        ax[row_idx, col_idx].set_ylim([ymin, ymax])
    plt.suptitle(suptitle, fontsize=24, y=0.96)
plt.show()
...
# initialize plot 
fig, ax = plt.subplots(num_secondary_effect_groups,len(var_to_plot), figsize=(num_primary_effect_groups*1.6*len(var_to_plot), num_secondary_effect_groups*5))
...
# initialize plot 
fig, ax = plt.subplots(num_secondary_effect_groups,len(var_to_plot), figsize=(num_primary_effect_groups*2*len(var_to_plot), num_secondary_effect_groups*3.8))
...
for col_idx, yvar in enumerate(var_to_plot): 
    ...
    for row_idx, secondary_effects in enumerate(secondary_effect_group_list):
        ...
        ax[row_idx, col_idx].set_ylabel(yvar, fontsize=18)
        ax[row_idx, col_idx].set_xlabel('', fontsize=16)
        ax[row_idx, col_idx].set_title(f'{secondary_effect_to_vary}: {secondary_effects}', fontsize=16)
        # ax[row_idx, col_idx].set_title(None)
        ax[row_idx, col_idx].set_ylim([ymin, ymax])
...
# initialize plot 
fig, ax = plt.subplots(num_secondary_effect_groups,len(var_to_plot), figsize=(num_primary_effect_groups*2*len(var_to_plot), num_secondary_effect_groups*5))
